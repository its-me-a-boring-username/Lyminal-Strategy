...
// Returns a solid light pastel by blending 15% sphere color into white
function paleSphereColor(hex) {
  if (!hex || hex.length < 7) return "#f5f0ec";
  const r = parseInt(hex.slice(1, 3), 16);
  const g = parseInt(hex.slice(3, 5), 16);
  const b = parseInt(hex.slice(5, 7), 16);
  return `rgb(${Math.round(0.85*255+0.15*r)},${Math.round(0.85*255+0.15*g)},${Math.round(0.85*255+0.15*b)})`;
}

// Returns a soft border color by blending 30% sphere color into white
function paleSphereColorBorder(hex) {
  if (!hex || hex.length < 7) return "#e8e0d5";
  const r = parseInt(hex.slice(1, 3), 16);
  const g = parseInt(hex.slice(3, 5), 16);
  const b = parseInt(hex.slice(5, 7), 16);
    ...
    <div className="min-h-screen lg:flex" style={{ background: BG, fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="w-full lg:flex-1 lg:flex lg:flex-col">
        ...
        <div className="max-w-4xl mx-auto px-4 lg:px-16" style={{ paddingTop: "16px", paddingBottom: "24px" }}>
          ...
          {/* Free preview banner */}
          {!isPaid && (
            <button onClick={() => setAuthPrompt("upgrade")} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px", marginBottom: "14px", padding: "9px 14px", background: "rgba(160,92,40,0.08)", border: `1px solid rgba(160,92,40,0.25)`, borderRadius: "8px", width: "100%", cursor: "pointer", textAlign: "left" }}>
              <p style={{ fontSize: "11px", color: MUTED, margin: 0, lineHeight: 1.4, fontFamily: "'Inter', sans-serif" }}>
                <span style={{ fontWeight: 600, color: COPPER }}>Preview mode.</span> Upgrade to use Forward, Schedule, and Find.
              </p>
              <span style={{ fontSize: "11px", fontWeight: 600, color: COPPER, flexShrink: 0, fontFamily: "'Inter', sans-serif" }}>Upgrade →</span>
            </button>
          )}
          ...
          <div style={{ display: "flex", gap: "8px", marginBottom: "14px", overflowX: "auto", WebkitOverflowScrolling: "touch", paddingBottom: "2px" }}>
            <button title="Show all action items" onClick={() => { setActiveType(null); setBulkSel(new Set()); }} style={{
              ...
              border: activeType === null ? `2px solid ${TC.none}` : `1px solid ${TC.none}`,
              background: activeType === null ? "transparent" : TC.none,
              color: activeType === null ? TC.none : "white",
            ...
            {ACTION_TYPES.map(t => {
              ...
              return (
                <button key={t} title={TITLES[t]} onClick={() => {
                  ...
                  borderRadius: "999px",
                  border: isAct ? `2px solid ${TC[t]}` : `1px solid ${TC[t]}`,
                  background: isAct ? "transparent" : TC[t],
                  ...
                  <div style={{ border: `1px solid ${BORDER_DARK}`, borderLeft: "none", borderTop: "none", background: "white", overflow: "hidden" }}>
                    <div>
                      {gFiltered.length === 0 ? (
                        ...
                        const factsOpen        = factsOpenId === item.id;

                        return (
                          <div key={item.id}>
                            <div style={{ display: "flex", alignItems: "flex-start", gap: "8px", padding: isMobile ? "12px 14px" : "14px 20px", borderBottom: hasSavedFindings && !factsOpen ? "none" : "1px solid #f0ebe3", background: isBulked ? TBG[item.type] : "white", transition: "background 0.15s" }}>
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                {/* Status badges row */}
                                {(item.schedule || item.forwardLogs?.length > 0) && (
                                ...
                                {/* Findings toggle button — prominent, shown on all screen sizes */}
                                {hasSavedFindings && (
                                  <button
                            ...
                            {/* Inline expand panel — same on mobile and desktop */}
                            {factsOpen && hasSavedFindings && (
                              <div style={{ background: TBG.find, borderTop: `1px solid ${TBORDER.find}`, borderBottom: "1px solid #f0ebe3", padding: "14px 20px" }}>
                                <p style={{ fontSize: "9px", textTransform: "uppercase", letterSpacing: "0.1em", color: TC.find, fontWeight: 600, fontFamily: "'Inter', sans-serif", margin: "0 0 10px" }}>Findings</p>
                                {item.findFacts[0].findings.map((f, fi) => (
                                  <div key={fi} style={{ display: "flex", gap: "8px", alignItems: "flex-start", marginBottom: "7px" }}>
                                    ...
                                    <p style={{ fontSize: "13px", color: "#1c1410", margin: 0, lineHeight: 1.5, fontFamily: "'Inter', sans-serif" }}>{processInline(f)}</p>
                                  </div>
                                ))}
                                {item.findFacts[0].closing && (
                                  <p style={{ fontSize: "11px", color: "#8a7455", fontStyle: "italic", fontFamily: "'Inter', sans-serif", margin: "8px 0 0" }}>{item.findFacts[0].closing}</p>
                                )}
                                <button onClick={() => openFindChat(g.goalId, [item])}
                                  style={{ marginTop: "12px", fontSize: "11px", fontWeight: 600, color: TC.find, background: "none", border: `1px solid ${TBORDER.find}`, padding: "6px 14px", cursor: "pointer", fontFamily: "'Inter', sans-serif", borderRadius: "4px" }}>
                                  Ask Lyme again →
                                </button>
                              </div>
                            )}
