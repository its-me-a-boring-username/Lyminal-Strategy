...
Do not ask follow-up questions after proposing action items unless the user wants to change something. Keep the whole conversation under 6 exchanges.`,
  ...
  return (
    <div className="flex flex-col overflow-hidden" style={{ height: "100dvh", background: isMobile ? "#26262c" : "#1c1c21", fontFamily: "'Inter', sans-serif" }}>
      <style>{FONTS}</style>
      <DevReset />
      {isMobile && <Nav step="chat" setStep={setStep} isMobile={isMobile} isPaid={isPaid} activeGoals={activeGoals} session={session} />}
      <div className="flex flex-col flex-1 overflow-hidden" style={{ paddingBottom: isMobile ? "60px" : 0, opacity: visible ? 1 : 0, transition: "opacity 0.3s ease-out" }}>

      {/* Header */}
      <div className="px-6 py-4 flex items-center gap-4" style={{ background: isMobile ? "#a05c28" : (isDefault ? hexToRgba(chatContext?.sphereColor, 0.07) : "rgba(var(--ly-accent-rgb), 0.07)"), borderBottom: isMobile ? "1px solid rgba(255,255,255,0.1)" : (isDefault ? `1px solid ${hexToRgba(chatContext?.sphereColor, 0.12)}` : "1px solid rgba(var(--ly-accent-rgb), 0.12)") }}>
        <div className="flex items-center gap-2 flex-1">
          ...
          <p className="text-sm font-medium" style={{ color: "#f0ebe2", margin: 0 }}>{chatContext?.goalText}</p>
        </div>
        <button onClick={() => setStep("active")} className="text-xs px-4 py-2 font-medium hover:opacity-80 transition-opacity"
          style={{ border: "1px solid rgba(255,255,255,0.25)", color: "rgba(255,255,255,0.8)", borderRadius: "6px", flexShrink: 0 }}>
          Exit
        </button>
      </div>
