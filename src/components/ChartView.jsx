          ...
          return (
            ...
            >
              ...
              <text
                ...
                fill={isSelected ? "rgba(255,255,255,0.9)" : "rgba(0,0,0,0.3)"}
              >
                ↑{c.in} ↓{c.out}
              </text>
              {isTop && (
...
const SidePanel = ({ spheres, connections, counts, ranked, selectedId, setSelectedId }) => {
  ...
  if (selected) {
    return (
      <div className="w-full lg:w-80 border-t lg:border-t-0 lg:border-l overflow-y-auto" style={{ background: "#26262c", borderColor: "#3a3a40" }}>
        <div style={{ padding: "24px", fontFamily: "'Inter', sans-serif" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "20px" }}>
            ...
            <button onClick={() => setSelectedId(null)} style={{ marginLeft: "auto", background: "none", border: "none", cursor: "pointer", color: "#888690", fontSize: "14px", padding: "2px 6px" }}>✕</button>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "20px" }}>
            <div style={{ background: hexToRgba(selected.color, 0.07), border: `1px solid ${hexToRgba(selected.color, 0.12)}`, borderRadius: "8px", padding: "12px", textAlign: "center" }}>
              <div style={{ fontSize: "1.5rem", fontWeight: 700, color: "#f0ebe2", fontFamily: "'Playfair Display', serif" }}>{selectedCounts?.out || 0}</div>
              <div style={{ fontSize: "11px", color: "#888690", marginTop: "2px" }}>Outgoing</div>
              <div style={{ fontSize: "10px", color: "#606068" }}>supports others</div>
            </div>
            <div style={{ background: hexToRgba(selected.color, 0.07), border: `1px solid ${hexToRgba(selected.color, 0.12)}`, borderRadius: "8px", padding: "12px", textAlign: "center" }}>
              <div style={{ fontSize: "1.5rem", fontWeight: 700, color: "#f0ebe2", fontFamily: "'Playfair Display', serif" }}>{selectedCounts?.in || 0}</div>
              <div style={{ fontSize: "11px", color: "#888690", marginTop: "2px" }}>Incoming</div>
              <div style={{ fontSize: "10px", color: "#606068" }}>needs support</div>
            </div>
          </div>
          {(connections[selected.id] || []).length > 0 && (
            <div style={{ marginBottom: "16px" }}>
              <p style={{ fontSize: "10px", color: "#888690", textTransform: "uppercase", letterSpacing: "0.08em", fontWeight: 600, margin: "0 0 8px" }}>Supports</p>
          ...
          {selected.goals.length > 0 && (
            <div>
              <p style={{ fontSize: "10px", color: "#888690", textTransform: "uppercase", letterSpacing: "0.08em", fontWeight: 600, margin: "0 0 8px" }}>Goals ({selected.goals.length})</p>
              {selected.goals.map(g => (
                <div key={g.id} style={{ display: "flex", alignItems: "flex-start", gap: "8px", fontSize: "13px", color: "#c8c0b4", paddingBottom: "8px", marginBottom: "8px", borderBottom: "1px solid #3a3a40" }}>
  ...
  return (
    <div className="w-full lg:w-80 border-t lg:border-t-0 lg:border-l overflow-y-auto" style={{ background: "#1c1c21", borderColor: "#3a3a40" }}>
      <div style={{ padding: "24px", fontFamily: "'Inter', sans-serif" }}>
        <p style={{ fontSize: "10px", letterSpacing: "0.1em", textTransform: "uppercase", color: "#888690", fontWeight: 600, margin: "0 0 4px" }}>Priority Ranking</p>
        <p style={{ fontSize: "11px", color: "#888690", margin: "0 0 16px" }}>Ranked by how many areas each sphere supports</p>
          ...
          {ranked.map((b, i) => (
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: "13px", fontWeight: 600, color: "#f0ebe2", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{b.name}</div>
                <div style={{ fontSize: "10px", color: "#888690" }}>↓{b.out} out · ↑{b.in} in</div>
              </div>
              {i === 0 && (
        ...
        <div style={{ marginTop: "20px", background: "rgba(74,124,142,0.1)", borderLeft: "3px solid #4a7c8e", borderRadius: "0 6px 6px 0", padding: "10px 14px" }}>
          <p style={{ fontSize: "11px", fontWeight: 600, color: "#f0ebe2", margin: "0 0 4px" }}>How to read this</p>
          <p style={{ fontSize: "11px", color: "#888690", lineHeight: 1.55, margin: 0 }}>
...
export function ChartView({
  ...
  return (
    ...
    <div className="min-h-screen" style={{ background: isMobile ? "#26262c" : "#1c1c21", fontFamily: "'Inter', sans-serif" }}>

      {/* Header */}
      <div style={{ opacity: visible ? 1 : 0, transition: "opacity 0.3s ease-out" }}>
      <div style={{ background: isMobile ? "#a05c28" : "#1c1c21", borderBottom: isMobile ? "1px solid rgba(255,255,255,0.1)" : "1px solid #3a3a40", padding: "16px 24px", fontFamily: "'Inter', sans-serif" }}>
        {/* Title row with inline CTA */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px", marginBottom: "10px" }}>
          <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "1.5rem", fontWeight: 600, color: "#f0ebe2", margin: 0 }}>Your Goal Chart</h2>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
          <button onClick={() => { setDragOffsets({}); setStep("connections"); }} style={{ fontSize: "11px", fontWeight: 500, color: "white", background: isMobile ? "rgba(255,255,255,0.2)" : "#a05c28", border: isMobile ? "1px solid rgba(255,255,255,0.3)" : "none", padding: "6px 12px", cursor: "pointer", borderRadius: "6px" }}>← Edit</button>
          <button onClick={handleRedoChart} style={{ fontSize: "11px", fontWeight: 500, color: isMobile ? "white" : "#a05c28", background: isMobile ? "rgba(255,255,255,0.15)" : "rgba(160,92,40,0.10)", border: isMobile ? "1px solid rgba(255,255,255,0.3)" : "1px solid rgba(160,92,40,0.35)", padding: "6px 12px", cursor: "pointer", borderRadius: "6px" }}>↺ Redo</button>
          {Object.keys(dragOffsets).length > 0 && (
            <button onClick={() => setDragOffsets({})} style={{ fontSize: "11px", fontWeight: 500, color: "#4a7c8e", background: "rgba(74,124,142,0.08)", border: "1px solid rgba(74,124,142,0.4)", padding: "6px 12px", cursor: "pointer", borderRadius: "6px" }}>↺ Reset layout</button>
          )}
        </div>
      </div>
      </div>
